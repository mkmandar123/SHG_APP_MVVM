...
object ImageUploaderUsersavingrequestCreate {
     ...
     */
    fun enqueue() {
        Log.d(TAG, "enqueue: enqueue in ImageUploaderUsersavingrequestCreate class is called.")
        val constraints =
            Constraints.Builder().setRequiredNetworkType(NetworkType.CONNECTED).build()

        val request: OneTimeWorkRequest =
            OneTimeWorkRequestBuilder<ImageUploadWorkerUsersavingrequestCreate>()
                .setConstraints(constraints)
                .addTag(ImageUploadWorkerUsersavingrequestCreate.TAG)
