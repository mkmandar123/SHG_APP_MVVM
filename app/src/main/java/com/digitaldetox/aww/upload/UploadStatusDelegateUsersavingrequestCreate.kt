...
import com.digitaldetox.aww.repository.main.CreateUsersavingrequestRepository
...
@Singleton
class UploadStatusDelegateUsersavingrequestCreate @Inject constructor() : net.gotev.uploadservice.UploadStatusDelegate {

    @Inject
    lateinit var albumDetailsRepository: CreateUsersavingrequestRepository
...
//            val image = Gson().fromJson<ImgurResponse<SubredditRoom>>(response.bodyAsString)
            val image = Gson().fromJson<UsersavingrequestRoom>(response.bodyAsString)
